...
public class ProductServiceTests {
    ...
    public void get_all_product_test(){
        ...
        // Verification
        assertEquals(foundProducts.get(0).getName(), productName);
    ...
    public void get_all_product_by_category_test(){
        ...
        assertEquals(products.get(0).getName(), PRODUCT_NAME);
    ...
    public void get_all_products_by_name_test(){
        // Data preparation
        Mockito.when(productRepository.findAllByName(PRODUCT_NAME)).thenReturn(products);

        //Method call
        List<Product> foundProducts = productService.getAllProductsByName(PRODUCT_NAME);

        //Verification
        assertEquals(foundProducts.get(0).getName(), PRODUCT_NAME);
        Mockito.verify(productRepository, Mockito.times(1)).findAllByName(Mockito.anyString());
        Mockito.verifyNoMoreInteractions(productRepository);
    }
